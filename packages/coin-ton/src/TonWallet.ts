...
export class TonWallet extends BaseWallet {
    ...
    async getNewAddress(param: NewAddressParams & {walletVersion?: string}): Promise<any> {
        try {
            ...
            if (!walletVersion || walletVersion.toLowerCase() == "v5r1") {
                address = getV5R1AddressBySeed(param.privateKey.toLowerCase());
...
export class TestTonWallet extends TonWallet {
    async getNewAddress(param: NewAddressParams & {walletVersion?: string}): Promise<any> {
        try {
            if (!base.validateHexString(param.privateKey)) {
                return Promise.reject(NewAddressError);
            ...
            let address: string
            if (!walletVersion || walletVersion.toLowerCase() == "v5r1") {
                address = getV5R1AddressBySeed(param.privateKey.toLowerCase(), true);
            } else if (walletVersion.toLowerCase() == "v4r2") {
                address = getAddressBySeed(param.privateKey.toLowerCase(), true);
            } else {
                throw NewAddressError
                // todo
            }
