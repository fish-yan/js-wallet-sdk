...
export function transfer(txData: TxData, seed: string) {
    ...
    if (!walletVersion || walletVersion.toLowerCase() == "v5r1") {
        ...
        let wallet = WalletContractV4.create({workchain: 0, publicKey: Buffer.from(publicK)});
        signedMessage = wallet.createTransfer({
            seqno: txData.seqno,
            messages,
            secretKey: seed ? Buffer.from(secretK!) : Buffer.alloc(0),
...
export function jettonTransfer(txData: JettonTxData, seed: string) {
    ...
    let signedMessage: Cell
    if (!walletVersion || walletVersion.toLowerCase() == "v5r1") {
        signedMessage = (wallet! as WalletContractV5R1).createTransfer({
            seqno: txData.seqno,
        ...
        signedMessage = (wallet! as WalletContractV4).createTransfer({
            seqno: txData.seqno,
            messages: internalMessage,
            secretKey: seed ? Buffer.from(secretK!) : Buffer.alloc(0),
            sendMode: txData.sendMode,
