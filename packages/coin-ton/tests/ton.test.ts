import {ApiTonConnectProof, TestTonWallet, TonWallet, VenomWallet} from "../src";
import {
    generateMnemonic,
    isPasswordNeeded,
    mnemonicToKeyPair,
...
const testWallet = new TestTonWallet();
...
describe("ton", () => {
    ...
    test("derive seed", async () => {
        const seed = await tonWallet.getDerivedPrivateKey({
            ...
            // hdPath: "m/44'/607'/0'", // for bip39 mnemoric
    ...
    test("ton getNewAddress", async () => {
        ...
        let result = await testWallet.getNewAddress({privateKey: seed, walletVersion: "v4r2"});
    ...
    test("getWalletInformation", async () => {
        const param = {
            ...
            privateKey: "4789815d45fcf0a79083b0adaec3b5f0d02e948056c1cfde174327c3ee93ed35",
            publicKey: "",
            walletVersion: "v5r1" // version of wallet, default v5r1
        };
        const walletInfo = await testWallet.getWalletInformation(param);
        const expected = {
            "initData": "te6cckEBAQEAKwAAUQAAAAApqaMXVPghZdNqsBolLipOppNSrl/0/yY+VopGvreUyr8MufRAc1cSdg==",
            "walletAddress": "0QA8JwrQ0gWhdmsl5UNLsz7EkLKwiXzILoCVUvFUZCCVZQMd",
